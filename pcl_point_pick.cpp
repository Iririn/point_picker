...
private:
	...
	// Extra function added by Weilun
	void saveIndex(std::vector<pcl::PointXYZ> m_registration_points);
	void searchIndex(pcl::PointXYZ search_point, cloud_ptr_t cloud, std::ofstream& out);
	std::string toString(pcl::PointXYZ search_point);
	...
	// Extra variable added by Weilun
	std::string filename;
	std::ofstream out;
};
...
bool PointPicker::loadMesh(std::string filename) {
	...
	// Load OBJ
	if (pcl::io::loadPolygonFileOBJ(filename, m_mesh) == -1) //* load the file
			{
...
void PointPicker::setSaveEnabled(bool enabled) {
        ...
        cout << "Saveing: " << (save_enabled ? "ON" : "OFF") << endl;
}
...
void PointPicker::keyboardCallback(const pcl::visualization::KeyboardEvent& event, void* cookie)
{
	...
	if (event.keyUp()) {
		switch (keycode) {
		case 's':
			saveIndex(m_registration_points);
			//setSaveEnabled(true);
		case 't':
			setPickEnabled(true);
			break;
...
void PointPicker::saveIndex(std::vector<pcl::PointXYZ> m_registration_points) 
{
	pcl::PointXYZ search_point;
	cloud_ptr_t cloud(new cloud_t);
	pcl::fromPCLPointCloud2(m_mesh.cloud, *cloud);

	filename = "FeaturePoints.txt";
	out.open(filename.c_str());
	
	for (int i = 0; i < m_registration_points.size(); i++) {
		search_point = m_registration_points[i];
		searchIndex(search_point, cloud, out);
	}
...
void PointPicker::searchIndex(pcl::PointXYZ search_point, cloud_ptr_t cloud, std::ofstream& out)
{
	for (int i = 0; i < cloud->size(); i++) {
		std::string search_line = toString(search_point);
		std::string target_line = toString(cloud->at(i));

		if (search_line == target_line) {
			out << "Point (" << search_point.x << " " << search_point.y << " " << search_point.z << ") at index: " << i+1 << "\n"; 
		}
	}
}

std::string PointPicker::toString(pcl::PointXYZ search_point)
{
	float x, y, z;
	int x_temp, y_temp, z_temp;

	x_temp = search_point.x * 1000;
	x = (float)x_temp / 1000;

	y_temp = search_point.y * 1000;
	y = (float)y_temp / 1000;

	z_temp = search_point.z * 1000;
	z = (float)z_temp / 1000;
...
void PointPicker::estimatePlaneNormal() {
	...
	std::cout << m_plane_coeff.values[3] << std::endl;
}
